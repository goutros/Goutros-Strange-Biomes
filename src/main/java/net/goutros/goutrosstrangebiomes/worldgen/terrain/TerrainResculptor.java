...
/**
 * WORKING TERRAIN RESCULPTOR - Uses actual biome detection
 */
public class TerrainResculptor {
    ...
    public static void resculptTerrain(ChunkAccess chunk, RandomState randomState) {
        ...
        // FIXED: Use actual biome detection instead of climate approximation
        BiomeAnalysis analysis = analyzeBiomeInfluence(chunk, chunkPos);

        if (analysis.hasCustomInfluence()) {
            LOGGER.info("Resculpting terrain for chunk {}, {} (influence: {:.1f}%)",
    ...
    /**
     * FIXED: Calculate biome influence using ACTUAL biome detection
     */
    private static BiomeInfluenceData calculateBiomeInfluence(ChunkAccess chunk, int worldX, int worldZ) {
        ...
        for (int dx = -BLEND_RADIUS; dx <= BLEND_RADIUS; dx += 4) {
            for (int dz = -BLEND_RADIUS; dz <= BLEND_RADIUS; dz += 4) {
                ...
                // FIXED: Use actual biome detection
    ...
    /**
     * FIXED: Check if a world position is in a Pillow Plateau biome using ACTUAL biome data
     */
    private static boolean isPillowBiomeAt(ChunkAccess chunk, int worldX, int worldZ) {
        try {
            ChunkPos chunkPos = chunk.getPos();

            // Handle positions outside chunk bounds by checking neighboring chunks if needed
            if (worldX < chunkPos.getMinBlockX() || worldX > chunkPos.getMaxBlockX() ||
                    worldZ < chunkPos.getMinBlockZ() || worldZ > chunkPos.getMaxBlockZ()) {
                return false; // For now, assume no influence outside chunk
            }

            int chunkX = worldX - chunkPos.getMinBlockX();
            int chunkZ = worldZ - chunkPos.getMinBlockZ();
            int biomeX = chunkX >> 2;  // Convert to biome coordinates
            int biomeZ = chunkZ >> 2;

            // Get actual biome at this position
            ResourceKey<Biome> biome = chunk.getNoiseBiome(biomeX, 16, biomeZ)
                    .unwrapKey().orElse(null);

            boolean isPillow = biome != null && biome.equals(ModBiomes.PILLOW_PLATEAU);

            if (isPillow) {
                LOGGER.debug("Found Pillow biome at world pos {}, {} (chunk {}, {})",
                        worldX, worldZ, chunkPos.x, chunkPos.z);
            }

            return isPillow;
        } catch (Exception e) {
            LOGGER.debug("Error checking biome at {}, {}: {}", worldX, worldZ, e.getMessage());
            return false;
        }
    }
    ...
    private static PlateauData generatePlateauTerrain(int worldX, int worldZ, int vanillaHeight) {
        // Layer selection - this creates the stepped effect
        ...
        // Add smooth variation within each layer
        ...
        int detailVariation = (int) ((detailNoise - 0.5) * 6); // ±3 blocks

        // Erosion for cliff faces
        ...
        double erosionFactor = Math.pow(erosionNoise, 2.0);

        if (erosionFactor < 0.3) {
            // Drop to lower layer
            int lowerLayerIndex = Math.max(0, layerIndex - 1);
            baseHeight = PLATEAU_LAYERS[lowerLayerIndex];
        } else if (erosionFactor > 0.85) {
            // Rise to higher layer
            int higherLayerIndex = Math.min(PLATEAU_LAYERS.length - 1, layerIndex + 1);
            baseHeight = PLATEAU_LAYERS[higherLayerIndex];
        }

        int finalHeight = baseHeight + detailVariation;

        // Determine if this should be an island (raised from ocean)
        boolean isIsland = vanillaHeight < SEA_LEVEL && layerNoise > 0.4;
    ...
    /**
     * Resculpt the actual blocks in a column - COMPLETE REPLACEMENT for high influence
     */
    private static void resculptColumnBlocks(ChunkAccess chunk, int x, int z, int vanillaHeight,
        ...
        // For high influence, completely replace terrain
        if (influence.influence > 0.6) {
            for (int y = minY; y < maxY; y++) {
                ...
                if (y <= targetHeight) {
                    // Solid terrain with colorful blocks
            ...
            // Low influence - modify existing terrain
            ...
            for (int y = modifyStart; y < modifyEnd; y++) {
                BlockPos pos = new BlockPos(x, y, z);
                BlockState newState = getTerrainBlock(y, targetHeight, plateau, influence);
                chunk.setBlockState(pos, newState, false);
            }
        }
    }
    ...
    private static BlockState getTerrainBlock(int y, int surfaceHeight, PlateauData plateau, BiomeInfluenceData influence) {
        ...
        if (influence.influence > 0.8) {
            // High influence - dramatic colored terrain
    ...
    /**
     * Get plateau block for high-influence areas - COLORFUL LAYERED BLOCKS
     */
    private static BlockState getPlateauBlock(int depthFromSurface, int materialLayer) {
        if (depthFromSurface == 0) {
            // Surface layer - dramatic colors matching reference
